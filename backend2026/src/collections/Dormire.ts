// src/collections/Dormire.ts
import type { CollectionConfig } from 'payload'

const slugify = (input: string) =>
  (input ?? '')
    .toString()
    .trim()
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 120)

export const Dormire: CollectionConfig = {
  slug: 'dormire',
  admin: {
    useAsTitle: 'title',
    labels: {
      singular: 'Struttura ricettiva',
      plural: 'Dormire',
    },
    defaultColumns: ['title', 'borgo', 'updatedAt'],
  },
  access: {
    read: () => true,
    create: ({ req }) => Boolean(req.user),
    update: ({ req }) => Boolean(req.user),
    delete: ({ req }) => Boolean(req.user),
  },
  fields: [
    {
      name: 'title',
      label: 'Nome struttura',
      type: 'text',
      required: true,
    },
    {
      name: 'slug',
      label: 'Slug',
      type: 'text',
      unique: true,
      admin: { description: 'Se vuoto, viene generato automaticamente dal nome' },
      hooks: {
        beforeValidate: [
          ({ data }) => {
            if (!data) return
            if (!data.slug && data.title) data.slug = slugify(data.title)
            if (typeof data.slug === 'string') data.slug = slugify(data.slug)
          },
        ],
      },
    },
    {
      name: 'borgo',
      label: 'Borgo',
      type: 'relationship',
      relationTo: 'borghi',
      required: true,
    },
    {
      name: 'descrizioneBreve',
      label: 'Descrizione breve',
      type: 'textarea',
      admin: { description: 'Massimo 300 caratteri' },
      validate: (val: unknown) => {
        if (!val) return true
        if (typeof val !== 'string') return 'Testo non valido'
        return val.length <= 300 || 'Max 300 caratteri'
      },
    },
    {
      name: 'immagineCopertina',
      label: 'Immagine di copertina',
      type: 'upload',
      relationTo: 'media',
    },
    {
      name: 'immaginiGalleria',
      label: 'Galleria immagini',
      type: 'array',
      fields: [
        {
          name: 'immagine',
          label: 'Immagine',
          type: 'upload',
          relationTo: 'media',
          required: true,
        },
        {
          name: 'did',
          label: 'Didascalia',
          type: 'text',
        },
      ],
    },
    {
      name: 'stanze',
      label: 'Stanze',
      type: 'array',
      admin: { description: 'Elenco delle stanze con prezzi e dettagli' },
      fields: [
        { name: 'nome', label: 'Nome stanza', type: 'text', required: true },
        {
          name: 'prezzo',
          label: 'Prezzo (EUR)',
          type: 'number',
          admin: { description: 'Prezzo base/notte' },
        },
        { name: 'descrizione', label: 'Descrizione', type: 'textarea' },
        {
          name: 'immagini',
          label: 'Immagini stanza',
          type: 'array',
          fields: [
            {
              name: 'immagine',
              label: 'Immagine',
              type: 'upload',
              relationTo: 'media',
              required: true,
            },
            { name: 'did', label: 'Didascalia', type: 'text' },
          ],
        },
      ],
    },
  ],
}
